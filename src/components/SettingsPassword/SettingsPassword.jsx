...
export const SettingsPassword = () => {
  ...
  const renderError = () => {
    ...
    const errorMessage = error?.response?.data.errors?.password?.[0] || 'An error occurred';
    return (
  ...
  return (
    ...
    >
      ...
      <div className={styles.passwordWrapper}>
        ...
        <div className={styles.passwordInputWrapper}>
          <input type="password" onChange={(e) => handlePasswordConfirmChange(e.target.value)}/>
        </div>
      </div>
      <div className={styles.divider}></div>
      {passError && (
        <h3 className={styles.error}>{passError}</h3>
      )}
      {successMessage && (
        <h3 className={styles.success}>{successMessage}</h3>
      )}
      {renderError()}
